...
class PublicFunnelController extends Controller
{
    public function resolve(Request $request, PublicSiteResolver $resolver, FunnelTrackingService $tracking): JsonResponse
    {
        ...
        if (! $funnel) {
            ...
            $funnel = $site ? Funnel::query()->where('site_id', $site->id)->where('slug', $key)->where('status', 'published')->first() : null;
        }
        if (! $funnel) {
            return response()->json(['message' => 'Not found.'], 404);
        }
        $step = $funnel->steps()->where('slug', $request->query('step', 'start'))->where('status', 'published')->first();
        if (! $step || ! is_array($step->published_content)) {
            return response()->json(['message' => 'Not found.'], 404);
        }
