...
it('resolves a platform subdomain after the stored apex changes', function () {
    ['user' => $user, 'workspace' => $workspace] = tenant();
    $headers = authHeaders($user, $workspace);
    $site = $this->withHeaders($headers)
        ->postJson('/api/v1/sites', ['name' => 'Moved Apex', 'subdomain' => 'movedapex'])
        ->assertCreated()
        ->json('data');

    \App\Models\Domain::query()->where('site_id', $site['id'])->update([
        'hostname' => 'movedapex.sites.oldplatform.test',
    ]);
    app(TenantCacheService::class)->invalidateSite(\App\Models\Site::query()->findOrFail($site['id']));

    $this->getJson('/api/v1/public/resolve?host=movedapex.sites.example.com')
        ->assertOk()
